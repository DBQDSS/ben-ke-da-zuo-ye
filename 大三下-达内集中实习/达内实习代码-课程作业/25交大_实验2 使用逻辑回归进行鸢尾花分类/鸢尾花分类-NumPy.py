...
class IrisLogisticRegression:
    ...
    def train(self, X_train, y_train, X_test, y_test, epochs=1000, lr=0.01):
        train_losses = []
        test_losses = []
        ...
        for epoch in range(epochs):
            ...
            dZ = y_pred
            dZ[range(num_samples), y_train] -= 1
            dZ /= num_samples
            ...
            # 计算测试集损失
            test_y_pred = self.forward(X_test)
            test_loss = self.cross_entropy_loss(test_y_pred, y_test)
        ...
        # 保存模型参数
        np.savez('model2.npz', weights=self.weights, bias=self.bias)
...
def visualize_model_performance(model, X_test, y_test, original_X_test):
    ...
    predicted_names = [species_names[i] for i in predicted]  # 转换为类别名称
...
def predict_single_from_test(model, X_test, y_test, original_X_test):
    ...
    try:
        ...
        sample = X_test[row_num].reshape(1, -1)
        ...
        print(f"预测类别: {label_map[predicted]}")
        print("预测概率分布:")
        probs = output[0]
...
def predict_user_input(model, scaler):
    ...
    try:
        ...
        # 预测
        output = model.forward(input_scaled)
        predicted = np.argmax(output, axis=1)[0]

        label_map = {0: 'Setosa', 1: 'Versicolor', 2: 'Virginica'}
        print(f"\n预测结果: 该鸢尾花属于 {label_map[predicted]}")
    except ValueError:
