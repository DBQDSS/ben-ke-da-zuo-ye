...
public class SortTest {
    ...
    public static double depth(Quicksort alg, int length, int T) {
        Double[] temparr = GenerateData.getRandomData(length);
        double totaldep = 0;
        for (int i = 0; i < T; i++) {
            alg.sort(temparr);
            totaldep += alg.getMaxDepth();
            alg.resetDepth(); // 重置递归深度
        }
        return totaldep / T;
    }
