...
public class LineXYDemo extends ApplicationFrame {
    ...
    public LineXYDemo(String title, sort.SortAlgorithm[] algorithms, int[] dataSizes, int testIterations) {
        super(title);
    ...
    private XYDataset createDatasetNew(sort.SortAlgorithm[] algorithms, int[] dataSizes, int testIterations) {
        XYSeriesCollection dataset = new XYSeriesCollection();

        for (sort.SortAlgorithm algorithm : algorithms) {
            XYSeries series = new XYSeries(algorithm.getClass().getSimpleName());
            for (int dataSize : dataSizes) {
                double averageTime = (sort.SortTest.testForRandom(algorithm, dataSize, testIterations));
        ...
        for (sort.SortAlgorithm algorithm : algorithms) {
            ...
            for (int dataSize : dataSizes) {
                double averageTime = (sort.SortTest.testForRandomSome(algorithm, dataSize, testIterations));
                //将横坐标做二的对数间隔（log2）；纵坐标作十的对数间隔处理（lg）
                series.add(Math.log(dataSize) / Math.log(2), Math.log10(averageTime));
            }
            dataset.addSeries(series);
    ...
    private XYDataset createDataset(sort.SortAlgorithm[] algorithms, int[] dataSizes, int testIterations) {
        XYSeriesCollection dataset = new XYSeriesCollection();

        for (sort.SortAlgorithm algorithm : algorithms) {
            XYSeries series = new XYSeries(algorithm.getClass().getSimpleName());
            for (int dataSize : dataSizes) {
                Double[] data = task4.GenerateData.getInversedData(dataSize);
                double averageTime = (sort.SortTest.testForDecsending(algorithm, dataSize, testIterations));
                //将横坐标做二的对数间隔（log2）；纵坐标作十的对数间隔处理（lg）
                series.add(Math.log(dataSize) / Math.log(2), Math.log10(averageTime));
            }
            dataset.addSeries(series);
    ...
    public static void main(String[] args) {
        //绘制七条图线，包括插入排序、选择排序、快速排序、归并排序与Shell、Hibbard、Knuth三种版本的希尔排序）
        SortAlgorithm[] algorithms = {new Insertion(), new Selection(), new Quicksort(),
                new Mergesort(), new Shell(), new Shell2(), new Shell3()};
        //横坐标取2的八次方到16次方共9个数
